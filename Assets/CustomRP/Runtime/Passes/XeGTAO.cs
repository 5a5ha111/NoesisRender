...
using static LightingPass;
using static XeGTAOResources;
...
#pragma warning restore CS0414
    ...
    void Render(RenderGraphContext context)
    {
        ...
        /*if (generateNormals)
        {
            ...
            cmd.SetComputeTextureParam(vaXeGTAO, GenerateNormalsKernel, _g_srcRawDepth, tempDepth);
            cmd.SetComputeTextureParam(vaXeGTAO, GenerateNormalsKernel, _g_outNormalmap, outNormalMap);


            cmd.DispatchCompute(vaXeGTAO, GenerateNormalsKernel, numTheadsX * 2, numTheadsY * 2, 1);


            cmd.SetGlobalTexture(_UintTextureNormals, outNormalMap);
            materialXeGTAOApply.SetTexture(_UintTextureNormals, outNormalMap);
            cmd.Blit(outNormalMap, colorTex, materialXeGTAOApply, _DebugNormals);
        }
        else
        {
            cmd.SetComputeTextureParam(vaXeGTAO, GTAOKernel, _g_srcNormalmapWorld, srcNormalMapWorld);
            cmd.SetGlobalTexture(_SourceTexture, srcNormalMapWorld);
            cmd.Blit(srcNormalMapWorld, colorTex, materialXeGTAOApply, _DebugNormalsWorld);
        }*/
    ...
    public static TextureHandle Record(RenderGraph renderGraph, Camera camera, in CameraRendererTextures textures, XeGTAOSettings xeGTAOsettings, XeGTAOTextures xeGTAOTextures, Vector2Int attachmentSize, Material applyXeGTAO, bool generateNormals, RenderTexture normalBuffer)
    {
        ...
        /*pass.tempDepth = builder.ReadWriteTexture(renderGraph.CreateTexture(tempDepth));
        pass.depthfilteredMip0 = builder.ReadWriteTexture(renderGraph.CreateTexture(descDepthMip0));
        pass.depthfilteredMip1 = builder.ReadWriteTexture(renderGraph.CreateTexture(descDepthMip1));
        pass.depthfilteredMip2 = builder.ReadWriteTexture(renderGraph.CreateTexture(descDepthMip2));
        pass.depthfilteredMip3 = builder.ReadWriteTexture(renderGraph.CreateTexture(descDepthMip3));
        pass.depthfilteredMip4 = builder.ReadWriteTexture(renderGraph.CreateTexture(descDepthMip4));*/
        ...
        GTAOUpdateConstants(ref gTAOConstants, safeSize.x, safeSize.y, xeGTAOsettings, camera.farClipPlane, camera.nearClipPlane, Mathf.Deg2Rad * camera.fieldOfView, aspect, frameCounter: 0);

...
RWTexture2D<uint> g_GenericOutputDataUAV;
...
{
    float4x4             View;                   // a.k.a. world to view
    float4x4             ViewInv;                // a.k.a. view to world
    float4x4             Proj;                   // a.k.a. view to projection
    float4x4             ProjInv;                // a.k.a. projection to view
    float4x4             ViewProj;               // a.k.a. world to projection
    float4x4             ViewProjInv;            // a.k.a. projection to world
    float4x4             ReprojectionMatrix;     // see vaDrawAttributes::GlobalSettings::ReprojectionMatrix

    float4               WorldBase;              // global world position offset for shading; used to make all shading computation close(r) to (0,0,0) for precision purposes
    float4               PreviousWorldBase;      // same as WorldBase except a frame old (same old frame used to compute ReprojectionMatrix)
    float4               CameraDirection;
    float4               CameraRightVector;
    float4               CameraUpVector;
    float4               CameraWorldPosition;    // drawContext.Camera.GetPosition() - drawContext.WorldBase; WARNING: THIS DOES NOT CONTAIN THE JITTER: TODO: FIX IT
    float4               CameraSubpixelOffset;   // .xy contains value of subpixel offset used for supersampling/TAA (otheriwse knows as jitter) or (0,0) if no jitter enabled; .zw are 0

    float2               ViewportSize;           // ViewportSize.x, ViewportSize.y
    float2               ViewportPixelSize;      // 1.0 / ViewportSize.x, 1.0 / ViewportSize.y
    float2               ViewportHalfSize;       // ViewportSize.x * 0.5, ViewportSize.y * 0.5
    float2               ViewportPixel2xSize;    // 2.0 / ViewportSize.x, 2.0 / ViewportSize.y
    ...
    float2               FOVXY;                  // { camera.GetXFOV(), camera.GetYFOV() }
    float2               PixelFOVXY;             // { camera.GetXFOV() / camera.ViewportSize.x, camera.camera.GetYFOV() / camera.ViewportSize.y } // these 2 are actually identical because pixels are square
    ...
    float2               CameraJitterDelta;      // see vaDrawAttributes::GlobalSettings::CameraJitterDelta
    float2               Dummy0;
